...
const NotificationPanel = () => {
    ...
    const getNotifications = async () => {
        try {
            ...
            if (page === 0) {
                ...
                const newNotifications = response.data.content.filter((newItem) => !notificationList.some((existingItem) => existingItem.notiId === newItem.notiId));
                if (newNotifications.length > 0) {
                    setNotificationList((prevNotifications) => [...prevNotifications, ...newNotifications]);
                }
            }
    ...
    return (
        <div className="relative !h-fit">
            ...
            <div ref={dropdownContentRef} className="notification-dropdown z-20 w-[29rem] min-w-[28rem] max-w-[29rem] right-0 !h-fit bg-white divide-y divide-gray-100 rounded-lg shadow-lg">
                <div className="block px-4 py-2 font-medium text-center text-gray-700 rounded-t-lg bg-gray-50 text-lg">Thông báo</div>

                <div className="block !h-fit flex items-center space-x-5 px-4 py-2 font-medium text-center text-gray-700 text-sm">
                    <div className={`w-fit rounded-full px-3 py-2 cursor-pointer ${status === "all" ? "bg-emerald-50 text-emerald-500 hover:bg-emerald-100" : "hover:rounded-full hover:bg-gray-50"}`} onClick={() => setStatus("all")}>
                        Tất cả
                    </div>
                    <div className={`w-fit rounded-full px-3 py-2 cursor-pointer ${status !== "all" ? "bg-emerald-50 text-emerald-500 hover:bg-emerald-100" : "hover:rounded-full hover:bg-gray-50"}`} onClick={() => setStatus("unread")}>
                        Chưa đọc
                    </div>
                </div>

                <div className="max-h-[65vh] overflow-auto ">
                    <div className="!h-fit divide-y divide-gray-100">
                        {notificationList.map((notification, index) => (
                            <NotificationItem notification={notification} key={index} read={() => readNotification(notification.notiId)} />
                        ))}
                    </div>

                    <div className="block !h-fit py-2 text-sm font-medium text-center text-gray-900 rounded-b-lg bg-gray-50 hover:bg-gray-100">
                        {page === totalPages - 1 ? (
