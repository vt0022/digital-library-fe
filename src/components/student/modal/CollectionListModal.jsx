            ...
            <div className="overflow-y-auto max-h-[24rem] h-fit">
                {collectionList.map((collection, index) => (
                    <div className="flex items-center space-x-3 p-2 cursor-pointer hover:bg-gray-200 rounded-lg relative group" key={index} onClick={() => addToCollection(collection.collectionId)}>
                        {collection.thumbnails && <img alt="collection image" src={collection.thumbnails[0]} className="w-12 h-12 min-w-12 min-h-12 rounded-lg object-cover" />}

                        {!collection.thumbnails && <div className="w-12 h-12 min-w-12 min-h-12 bg-gray-300 rounded-lg" />}
